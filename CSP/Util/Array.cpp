...
ByteDynArray ByteArray::getASN1Tag(unsigned int tag) const {
	...
	ByteArray baTmp{result.mid(tl)};
	putASN1Length(size(), baTmp);
...
ByteDynArray &ByteDynArray::setASN1Tag(unsigned int tag, const ByteArray &content) {
	size_t tl = ASN1TLength(tag);
	size_t ll = ASN1LLength(content.size());
	resize(tl + ll + content.size());
	putASN1Tag(tag, *this);
	ByteArray baTmp{mid(tl)};
	putASN1Length(content.size(), baTmp);
