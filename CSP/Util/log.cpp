...
void initLog(const char *iniFile,const char *version) {
	...
	(IniSettingsString("Log", "LogDir", PATH_SEPARATOR_STR, "Definisce il path in cui salvare il file di log (con " PATH_SEPARATOR_STR " finale)")).GetValue((char*)iniFile, logDirGlobal);
	...
	while (true) {
		...
		(IniSettingsString(SectionName, "LogDir", logDirGlobal.c_str(), "Definisce il path in cui salvare il file di log di questa sezione (con " PATH_SEPARATOR_STR " finale). Default: directory di log globale")).GetValue((char*)iniFile, log.logDir);

		(IniSettingsString(SectionName, "LogFile", log.logName.c_str(), "Definisce il nome del file in cui salvare il file di log di questa sezione (con " PATH_SEPARATOR_STR " finale). Default: il nome della sezione di log")).GetValue((char*)iniFile, log.logFileName);
...
void CLog::initParam(CLog &log) {
	...
	GetLocalTime(&stTime);
#else
	time_t sFrom = time_t(nullptr);
	struct tm *stTime = localtime( &sFrom );
#endif
	...
	if ((LogMode==LM_Module || LogMode==LM_Module_Thread) && log.logDir.length()!=0) {
		...
		logPath=log.logDir.append(PATH_SEPARATOR_STR).append(path);
	}
	else if (!logDirGlobal.empty()) {
		// se c'è un path globale lo metto lì
		std::string path=logPath;
		logPath = logDirGlobal.append(PATH_SEPARATOR_STR).append(path);
	}
...
DWORD CLog::write(const char *format,...) {
	...
	if (Enabled && Initialized && MainEnable) {
		...
		sprintf(pbtDate,"%05u:[%02d:%02d:%02d.%03d]", *Num, stTime->tm_hour, stTime->tm_min, stTime->tm_sec, 0);	
#endif
