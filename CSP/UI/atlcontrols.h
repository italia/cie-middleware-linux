...
// Constructors
  ...
  BOOL Create(UINT nBitmapID, int cx, int nGrow, COLORREF crMask)
  {
    ...
    m_hImageList = ImageList_LoadBitmap(_AtlModule.GetModuleInstance(), MAKEINTRESOURCE(nBitmapID), cx, nGrow, crMask);
    return (m_hImageList != NULL) ? TRUE : FALSE;
  }
  BOOL Create(LPCTSTR lpszBitmapID, int cx, int nGrow, COLORREF crMask)
  {
    ATLASSERT(m_hImageList == NULL);
    m_hImageList = ImageList_LoadBitmap(_AtlModule.GetModuleInstance(), lpszBitmapID, cx, nGrow, crMask);
    return (m_hImageList != NULL) ? TRUE : FALSE;
  }
...
// Operations
  ...
  int AddBitmap(int nNumButtons, UINT nBitmapID)
  {
    ATLASSERT(::IsWindow(m_hWnd));
    TBADDBITMAP tbab;
    tbab.hInst = _AtlModule.GetResourceInstance();
  ...
  {
    ATLASSERT(::IsWindow(m_hWnd));
    return (int)::SendMessage(m_hWnd, TB_ADDSTRING, (WPARAM)_AtlModule.GetResourceInstance(), (LPARAM)nStringID);
  }
...
//REVIEW - return value?
  ...
  {
    ATLASSERT(::IsWindow(m_hWnd));
    return (int)::SendMessage(m_hWnd, TB_LOADIMAGES, nBitmapID, (LPARAM)_AtlModule.GetResourceInstance());
  }
...
// Operations
  ...
  void UpdateTipText(LPCTSTR lpszText, HWND hWnd, UINT nIDTool = 0)
  {
    ...
    TOOLINFO ti;
    FillInToolInfo(ti, hWnd, nIDTool);
    ti.hinst = _AtlModule.GetResourceInstance();
    ti.lpszText = (LPTSTR)lpszText;
