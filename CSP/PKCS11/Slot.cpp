...
namespace p11 {
		...
		while (true) {
			...
			DWORD ris;
			{
				...
				for (SlotMap::const_iterator it = pSlotMap->begin(); it != pSlotMap->end(); it++, i++) {
					...
					slot[i] = it->second;
					if (ris = SCardGetStatusChange(Context, 0, &state[i], 1) != SCARD_S_SUCCESS) {
	...
	void CSlot::GetInfo(CK_SLOT_INFO_PTR pInfo)
	{
		...
		size_t SDLen = min(size_t{64}, szName.size());
		memcpy_s(pInfo->slotDescription, size_t{64}, szName.c_str(), SDLen);

		memset(pInfo->manufacturerID, 0, 32);

		size_t MIDLen = min(size_t{32}, szVendor.size());
		memcpy_s(pInfo->manufacturerID, size_t{32}, szVendor.c_str(), MIDLen);
