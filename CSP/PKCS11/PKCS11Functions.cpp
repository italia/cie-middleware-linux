...
//		checkOutArray(pSignature, pulSignatureLen)
	...
	ByteArray baData(pData, ulDataLen);
	pSession->Sign(baData, Signature);
...
//	checkInBuffer(pPart, ulPartLen)
	...
	ByteArray baPart(pPart, ulPartLen);
	pSession->SignUpdate(baPart);
...
//		checkOutArray(pSignature, pulSignatureLen)
	...
	ByteArray Signature(pSignature,*pulSignatureLen);
	ByteArray baData(pData, ulDataLen);
	pSession->SignRecover(baData, Signature);
	*pulSignatureLen = (CK_ULONG)Signature.size();
...
//		checkInBuffer(pSignature, ulSignatureLen)
	...
	ByteArray baSign(pSignature, ulSignatureLen);
	pSession->VerifyRecover(baSign, Data);
...
//		checkOutArray(pEncryptedData, pulEncryptedDataLen)
	...
	ByteArray baData(pData, ulDataLen);
	pSession->Encrypt(baData, EncryptedData);
...
//		checkOutArray(pEncryptedPart, pulEncryptedPartLen)
	...
	ByteArray baPart(pPart, ulPartLen);
	pSession->EncryptUpdate(baPart, EncryptedPart);
...
//		checkOutArray(pData, pulDataLen)
	...
	ByteArray Data(pData, *pulDataLen);
	ByteArray baEnc(pEncryptedData, ulEncryptedDataLen);
	pSession->Decrypt(baEnc, Data);
	*pulDataLen = (CK_ULONG)Data.size();
...
//		checkOutArray(pPart, pulPartLen)
	...
	ByteArray baEnc(pEncryptedPart, ulEncryptedPartLen);
	pSession->DecryptUpdate(baEnc, Part);
...
//	checkOutBuffer(RandomData, ulRandomLen)
	...
	ByteArray baRand(RandomData, ulRandomLen);
	pSession->GenerateRandom(baRand);
...
//	checkInBuffer(pNewPin,ulNewLen);
	...
	ByteArray baOldPin(pOldPin, ulOldLen);
	ByteArray baNewPin(pNewPin, ulNewLen);
	pSession->SetPIN(baOldPin, baNewPin);
...
//	checkInBuffer(pOperationState, ulOperationStateLen)
	...
	ByteArray baOp(pOperationState, ulOperationStateLen);
	pSession->SetOperationState(baOp);
