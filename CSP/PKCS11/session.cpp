...
namespace {
	...
	public:
		...
		resetter(resetter&& other) noexcept /*: m_p(std::exchange(other.m_p, nullptr))*/ {m_p = other.m_p; other.m_p = nullptr;}
		...
		{
			reset();
			m_p = other.m_p; other.m_p = nullptr;//m_p = std::exchange(other.m_p, nullptr);
			return *this;
		}
...
namespace p11 {
	...
	void CSession::Digest(const ByteArray &Data, ByteArray &Digest)
	{
		init_func
	...
	void CSession::DigestUpdate(const ByteArray &Data)
	{
		init_func
			if (pDigestMechanism==nullptr)
				throw p11_error(CKR_OPERATION_NOT_INITIALIZED);
