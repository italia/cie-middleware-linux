...
extern ByteArray SkipZero(const ByteArray &ba);
...
void IAS::DAPP() {
	...
	auto va = VarToByteArray(baseCHR);
	auto vb = VarToByteArray(snIFD);
	CHR.set(&va, &vb);
	CHA.set(&CA_AID,01);	
	uint8_t baseOID[] = { 0x2A, 0x81, 0x22, 0xF4, 0x2A, 0x02, 0x04, 0x01 };
	auto dynOid = VarToByteArray(baseOID);
	OID.set(&dynOid, shaOID);
	...
	uint8_t ValBC = 0xBC;
	auto dynEndCert = (endEntityCert.left(CA_module.size() - shaSize - 2));
	auto dynEndCertDigest = (sha256.Digest(endEntityCert));
	toSign.set(0x6A, &dynEndCert, &dynEndCertDigest, 0xbc); 
	...
	PRND.random();
	auto dynIfd = VarToByteArray(snIFD);
	toHash.set(&PRND, &dh_pubKey, &dynIfd, &challenge, &dh_ICCpubKey, &dh_g, &dh_p, &dh_q);

	auto dynHash = (sha256.Digest(toHash));	
	toSign.set(0x6a, &PRND, &dynHash, 0xBC);
	
	CRSA certKey(module, privexp);
	ByteDynArray signResp = certKey.RSA_PURE(toSign);
	ByteDynArray chResponse;
	auto dIfd = VarToByteArray(snIFD);
	chResponse.set(&dIfd, &signResp);
	...
	auto dynChall = (challenge.right(4));
	auto dynRnd = (rndIFD.right(4));
	sessSSC.set(&dynChall, &dynRnd);	
	ActiveSM = true;
	exit_func
}
...
StatusWord IAS::SendAPDU_SM(const ByteArray &head, const ByteArray &data, ByteDynArray &resp, uint8_t *le) {
	init_func
	ByteDynArray smApdu;
	...
	if (data.size() < 0xE7) {
		auto va = (ByteArray());
		auto vb = (VarToByteArray(*le));
		smApdu.set(&head, (uint8_t)data.size(), &data, le == nullptr ? &va : &vb);
		...
		while (true) {
			...
			auto vb = (VarToByteArray(*le));
			if (s.size() != 0)
				smApdu.set(&head, (BYTE)s.size(), &s, (le == nullptr || i < data.size()) ? &va : &vb);	
			else
				smApdu.set(&head, (le == nullptr || i < data.size()) ? &va : &vb);	
...
StatusWord IAS::SendAPDU(const ByteArray &head, const ByteArray &data, ByteDynArray &resp, uint8_t *le) {
	init_func
	...
	if (ds > 255) {
		...
		while (true) {
			...
			auto va = (ByteArray());
			auto vb = (VarToByteArray(*le));
			apdu.set(&head, (BYTE)s.size(), &s, le == nullptr ? &va : &vb);	
		...
		auto vb = (VarToByteArray(*le));
		if (data.size()!=0)			
			apdu.set(&head, (BYTE)data.size(), &data, le == nullptr ? &va : &vb);
		else
			apdu.set(&head, le == nullptr ? &va : &vb);
...
void IAS::SetCache(const char *PAN, const ByteArray &certificate, const ByteArray &FirstPIN) {
	init_func
...
#else
			std::cout << "Launching AbilitaCIE [PAN]" << std::endl;
			AbilitaCIE(nullptr, nullptr, PANStr.c_str()/*dumpHexData(PAN.mid(5, 6), std::string(), false)*/, 0);
#endif
...
void IAS::VerificaSOD(ByteArray &SOD, std::map<BYTE, ByteDynArray> &hashSet) {
	...
	temp.Child(0, 6).Verify(VarToByteArray(OID));
	uint8_t val3=3;
	...
	ByteArray serArr{pder, pderlen/*certDS->pCertInfo->SerialNumber.pbData, certDS->pCertInfo->SerialNumber.cbData*/};
	ByteDynArray certSerial{serArr};
	BN_free(bn);
	if (certSerial/*.reverse()*/ != signerCertSerialNumber.content)
	{
		delete[] pder;	
