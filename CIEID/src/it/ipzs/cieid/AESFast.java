...
//        }
//
////        LogViewer.logMsg("ciphertext " + Encoder.bytesToHexString(outsCipherText.toByteArray()));
//        
...
//        LogViewer.logMsg("blocksize " + getBlockSize());
        ...
        try
        {
            ...
            {
//            	LogViewer.logMsg("cipher block " + Encoder.bytesToHexString(ciphertext, nOffset, getBlockSize()));
                processBlock(ciphertext, nOffset, block, 0);
//                LogViewer.logMsg("clear block " + Encoder.bytesToHexString(block));
                outs.write(block, 0, getBlockSize());
                nOffset += getBlockSize();
            }
            
//            LogViewer.logMsg("to unpad " + Encoder.bytesToHexString(outs.toByteArray()));
...
//     * @return The ciphertext generated using the session key.
//     */
//    public static byte[] encryptECB(byte[] plaintext, int nOffset, byte[] key, int keylen, int nBlockLen)
//    {
...
//
//    public static byte[] encryptCBC(byte[] plaintext, int nOffset, byte[] key, int keylen, byte[] iv, int nBlockLen)
//    {
//    	AESFast aes = new AESFast();
//    	aes.init(true, makeSafeKey(key, keylen));
//
        ...
        {            
            if (nDiff > 0)
            {
                ...
                for(int i = 0; i < nDiff; i++)
                    outs.write(k[i % k.length]);
            }
            else if(nDiff < 0)
            {
            	int len = k.length + nDiff;
                for(int i = 0; i < k.length + nDiff; i++)
                    outs.write(k[i]);
            }     	
            else
            {
...
//    {
//        String msg = "ciao ciao, sto cercando di farlo funzionare e ora vedremo cosa succede perch� sembra che il problema � rijndael";
//        String key = "ugochirico12345";
//        String iv = "assadfghjkasdasdasdadsasdasdasdasdasfsdgsd";
//               
//        byte[] enc = encryptCBC(msg.getBytes(), 0, key.getBytes(), KEY_SIZE_256, iv.getBytes(), BLOCK_SIZE_128);
//        System.out.println(Encoder.bytesToHexString(enc));
//        
//        byte[] dec = decryptCBC(enc, 0, key.getBytes(), KEY_SIZE_256, iv.getBytes(), BLOCK_SIZE_128);
